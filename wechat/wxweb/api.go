...
// JsLogin: jslogin api
func JsLogin(client *http.Client, common *Common) (string, error) {
	km := url.Values{}
	...
	req.Header.Add("Referer", common.CgiDomain)
...
// QrCode: get qrcode
func QrCode(client *http.Client, common *Common, uuid string) ([]byte, error) {
	km := url.Values{}
	km.Add("t", "webwx")
	km.Add("_", strconv.FormatInt(time.Now().Unix(), 10))
...
// Login: login api
func Login(client *http.Client, common *Common, uuid, tip string) (string, error) {
	km := url.Values{}
	km.Add("tip", tip)
	km.Add("uuid", uuid)
...
func WebNewLoginPage(client *http.Client, common *Common, xc *XmlConfig, uri string) ([]*http.Cookie, error) {
	u, _ := url.Parse(uri)
	km := u.Query()
	km.Add("fun", "new")
...
func WebWxInit(client *http.Client, common *Common, ce *XmlConfig) ([]byte, error) {
	km := url.Values{}
...
func SyncCheck(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, server string, skl *SyncKeyList) (int, int, error) {
	km := url.Values{}
...
func WebWxSync(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, msg chan []byte, skl *SyncKeyList) error {
	//logs.Info("WebWxSync.skl",skl)
	km := url.Values{}
	km.Add("skey", ce.Skey)
	km.Add("sid", ce.Wxsid)
	km.Add("lang", common.Lang)
...
func WebWxStatusNotify(client *http.Client, common *Common, ce *XmlConfig, bot *User) (int, error) {
	km := url.Values{}
	km.Add("pass_ticket", ce.PassTicket)
	km.Add("lang", common.Lang)
...
func WebWxSendMsg(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie,
...
func WebWxUploadMedia(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie,
...
func WebWxSendMsgImg(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie,
...
func WebWxGetMsgImg(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, msgId string) ([]byte, error) {
	km := url.Values{}
	km.Add("MsgID", msgId)
	km.Add("skey", ce.Skey)
	km.Add("type", "slave")
...
func WebWxSendEmoticon(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie,
	from, to, media string) (int, error) {
...
func WebWxGetIcon(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie,
...
func WebWxGetIconByHeadImgUrl(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, headImgUrl string) ([]byte, error) {
...
func WebWxBatchGetContact(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, cl []*User) ([]byte, error) {
	km := url.Values{}
	km.Add("r", strconv.FormatInt(time.Now().Unix(), 10))
	km.Add("type", "ex")
...
func WebWxVerifyUser(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, verifyContent string, vul []*VerifyUser,opcode int) ([]byte, error) {
	//logs.Info("api.WebWxVerifyUser",verifyContent,*vul[0],opcode)
	km := url.Values{}
	km.Add("r", strconv.FormatInt(time.Now().Unix(), 10))
	km.Add("pass_ticket", ce.PassTicket)
	...
	resp, err := client.Do(req)
	if err != nil {
		logs.Info("WebWxVerifyUser error",err.Error())
		return nil, err
	}
	defer resp.Body.Close()
	body, _ := ioutil.ReadAll(resp.Body)
...
func WebWxCreateChatroom(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, users []*User, topic string) (interface{}, error) {
	km := url.Values{}
	km.Add("r", strconv.FormatInt(time.Now().Unix(), 10))
	km.Add("pass_ticket", ce.PassTicket)
	uri := common.CgiUrl + "/webwxgetcontact?" + km.Encode()
...
func WebWxRevokeMsg(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie, clientMsgId, svrMsgId, toUserName string) error {
	km := url.Values{}
	km.Add("lang", common.Lang)
...
func WebWxLogout(client *http.Client, common *Common, ce *XmlConfig, cookies []*http.Cookie) error {
	km := url.Values{}
	km.Add("redirect", "1")
	km.Add("type", "1")
	km.Add("skey", ce.Skey)
...
// 邀请好友入群
func InviteFriend(s *Session, group *User, usernames ...string) error {
	var fun = "invitemember"//"invitemember"
	var mode = "InviteMemberList"//"InviteMemberList"

	if mm,err := CreateMemberManagerFromGroupContact(s,group); err == nil {
		//如果群人数大于等于40,是invitemember否则是addmember
		if count := len(mm.Group.MemberList)+len(usernames); count < 40 {
			fun = "addmember"
			mode = "AddMemberList"
			//logs.Info("InviteFriend.count",count)
		}
	}else{
		return fmt.Errorf("InviteFriend.get group member err",err.Error())
	}

	var invitedUsernamesArg string
	if len(usernames) > 1{
		for _,v := range usernames {
			invitedUsernamesArg += fmt.Sprintf(",%s",v)
		}
		invitedUsernamesArg = invitedUsernamesArg[1:]
	}else{
		invitedUsernamesArg = usernames[0]
	}

	km := url.Values{}
	km.Add("fun", fun)
	km.Add("r", strconv.FormatInt(time.Now().Unix(), 10))
	km.Add("pass_ticket", s.WxWebXcg.PassTicket)

	uri := s.WxWebCommon.CgiUrl + "/webwxupdatechatroom?" + km.Encode()
	//logs.Info("url",uri)
	...
	data := map[string]interface{}{
		"BaseRequest": &BaseRequest{
			...
			s.WxWebXcg.Wxsid,
			s.WxWebXcg.Skey,
			s.WxWebCommon.DeviceID,
		},
		"ChatRoomName": group.UserName,
	}
	data[mode] = invitedUsernamesArg

	b, _ := json.Marshal(data)
	//logs.Info("InviteFriend.b",string(b))
	req, err := http.NewRequest("POST", uri, bytes.NewReader(b))
	if err != nil {
		return err
	}
	req.Header.Add("Content-Type", "application/x-www-form-urlencoded")
	req.Header.Add("User-Agent", s.WxWebCommon.UserAgent)
	req.Header.Add("Referer", s.WxWebCommon.CgiDomain)
...
// 邀请好友入群
func KickGroupMember(s *Session, group *User, memberUserName string) error {
	km := url.Values{}
	km.Add("fun", "delmember")
	km.Add("r", strconv.FormatInt(time.Now().Unix(), 10))
	km.Add("pass_ticket", s.WxWebXcg.PassTicket)
	uri := s.WxWebCommon.CgiUrl + "/webwxupdatechatroom?" + km.Encode()
	//logs.Info("url",uri)
